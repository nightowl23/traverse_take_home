...
logger = logging.getLogger("verifiers.envs.InfraHarborEnv")


class InfraHarborEnv(HarborEnv):
    """Harbor environment for DevOps/infrastructure configuration tasks.
    ...
    def __init__(
        ...
        agent_workdir: str = "/infra",
        docker_image: str = "python:3.11-slim",
        **kwargs,
    ):
        run_command = "python /infra/agent.py 2>&1"
    ...
    async def post_sandbox_setup(self, state: vf.State) -> None:
        ...
        try:
            await self.sandbox_client.upload_file(
                sandbox_id, "/infra/agent.py", tmp_path
            )
        finally:
...
SYSTEM_PROMPT = (
    "You are a skilled DevOps engineer. You have access to tools for running "
    "bash commands and editing files. Read /task/instruction.md to understand "
    "the infrastructure task, then complete it. The infrastructure project is "
    "at /infra/. Work step by step. When you are done, call no more tools."
)
